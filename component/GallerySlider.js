...
export const GallerySlider = ( {galleryData} ) => {
    ...
    const isOddSlide = currentSlide % 2 === 1; // Check if the current slide is odd
                                    ...
                                    {sliderContent && sliderContent.map((slide, index) => (
                                        <div key={index}>
                                            <div className={`gallerybanner position-relative ${isOddSlide ? 'zoom-in' : 'zoom-out'}`}>
                                                <Image src={slide.imageSrc} width='1500' height='1000' alt={slide.altText} />
                                            </div>
                                        </div>
                                    ))}
                                </Slider>
                                <div className="slider-numbers flex-center mt-2">
                                    {currentSlide ? currentSlide + 1 : null}/{sliderContent ? sliderContent.length : null}
                                </div>
                            </div>
                        </div>
                    </div>
